...
{
    return (flags & ~(zcashconsensus_SCRIPT_FLAGS_VERIFY_ALL)) == 0;
}
                                    ...
                                    unsigned int nIn, unsigned int flags, zcashconsensus_error* err)
{
    if (!verify_flags(flags)) {
        return zcashconsensus_ERR_INVALID_FLAGS;
    }
    try {
...
int zcashconsensus_verify_script_with_amount(const unsigned char *scriptPubKey, unsigned int scriptPubKeyLen, int64_t amount,
                                    const unsigned char *txTo        , unsigned int txToLen,
                                    unsigned int nIn, unsigned int flags, zcashconsensus_error* err)
{
    CAmount am(amount);
...
int zcashconsensus_verify_script(const unsigned char *scriptPubKey, unsigned int scriptPubKeyLen,
                                   ...
                                   unsigned int nIn, unsigned int flags, zcashconsensus_error* err)
{
    if (flags & zcashconsensus_SCRIPT_FLAGS_VERIFY_WITNESS) {
        return set_error(err, zcashconsensus_ERR_AMOUNT_REQUIRED);
    }
